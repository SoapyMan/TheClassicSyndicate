...
function MISSION.SpawnTrapCars()						-- Spawning NPC cars for trap
	...
	local carDefs = {
		{"hd_scimitar", 1, vec3(1720.20, 0.81, -2.77),vec3(179.77, 6.71, 179.97)},
		{"hd_toledosedan", 2, vec3(1707.65, 0.81, -10.81),vec3(-179.07, -63.55, 179.20)},
		{"hd_toledosedan", 0, vec3(1708.42, 0.81, -22.87),vec3(-0.05, -52.63, 0.06) },
		{"hd_toledosedan", 1, vec3(1732.00, 0.81, -18.50),vec3(-1.22, 82.14, -1.22)},
		{"hd_scimitar", 2, vec3(1727.43, 0.81, -3.09),vec3(-179.34, 39.84, -179.57)},
		{"hd_toledosedan", 0, vec3(1723.88, 0.81, -30.89),vec3(-0.32, 15.25, -0.09)}
	}
	...
	local cutCameras = {
		{
			...
			{ Vector3D.new(-88.00,2.11,-1136.50), Vector3D.new(0.0, 180.02, 0), 4.0, 60 },
		}
	}
...
function MISSION.Phase1Start()
	...
	missionmanager:SetPluginRefreshFunc("ResiCarOBJ", function()	-- Spawn scenery when player enters % radius
		...
		if length(playerCar:GetOrigin() - MISSION.Data.targetPosition) < 100 then	-- Set up function
			...
			local opponentCar = gameses:CreatePursuerCar("hd_scimitar", PURSUER_TYPE_GANG)
