...
world:SetEnvironmentName("night_dawn_transition")
...
MISSION.Init = function()
	MISSION.Data = {
		targetPosition = Vector3D.new(221.84,0.57,713.45),
		target2Position = Vector3D.new(515.88,0.57,121.76),
		target3Position = Vector3D.new(519.05,0.58,598.96)
	}
	...
	gameHUD:ShowAlert("CASE FOR A KEY", 3.5, HUD_ALERT_NORMAL)
...
function MISSION.Phase2Start()
	...
	missionmanager:EnableTimeout(true, 80)
	...
	if distToTarget < 80.0 then
		...
		elseif playerSpeed < 60 then
			...
			if distToTarget < 4.0 then
				...
				missionmanager:EnableTimeout( false )
				
				-- TODO: cutscene black bar curtains
				missionmanager:ScheduleEvent( MISSION.Phase2Start, 2.5 )
...
function MISSION.ChangeTargets()
	...
	gameHUD:ShowScreenMessage("The dealer cheated you, go after him on Dodge Island.", 3.5)
	
	gameHUD:RemoveTrackingObject(MISSION.targetHandle)

	missionmanager:EnableTimeout(true, 100)

	MISSION.MusicScript = "miami_day"
