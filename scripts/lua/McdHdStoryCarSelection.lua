...
local StoryCarsList = {
	{"hd_arvensis", "Miami - Default PSX Car Remake"},
	{"hd_gsx2", "Miami - Beta GSX - Remake"},
	{"hd_gsx", "Miami - Beta GSX - Remake (Beta sounds)"},	
}

function McdHdGetPlayerCarName()
	local storyPreferences = RestoreMissionCompletionData("McdHdStoryPreferences")
	if storyPreferences ~= nil then
		McdHdPrefferedStoryCar = storyPreferences.PrefferedStoryCar
	end
	
	-- reset in some paranoid cases
	if McdHdPrefferedStoryCar == nil then
		McdHdPrefferedStoryCar = "hd_arvensis"
	end
	
	return McdHdPrefferedStoryCar
end

local function McdHdCarSelectionElementsFunc(equiScheme, stack)

	-- reset car type if mods were disabled
	if MenuStack.FindChoiceIndex(StoryCarsList, McdHdPrefferedStoryCar) == -1 then
		McdHdPrefferedStoryCar = StoryCarsList[1][1]
	end
	...
	local elems = {
		MenuStack.MakeChoiceParam("< %s >", currentCarNameGetSet, StoryCarsList),
		{
			...
			onEnter = function(self, stack)
				...
				-- first store for MCD
				StoreMissionCompletionData("McdHdStoryPreferences", {
					PrefferedStoryCar = McdHdPrefferedStoryCar
				})
...
return McdHdCarSelectionElementsFunc